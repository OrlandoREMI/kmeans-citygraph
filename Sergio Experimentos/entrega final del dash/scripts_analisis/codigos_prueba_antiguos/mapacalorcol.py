...
bares       = ox.features_from_place(lugar, tags={'amenity': ['bar', 'pub', 'nightclub']})
atms        = ox.features_from_place(lugar, tags={'amenity': 'atm'})
bancos      = ox.features_from_place(lugar, tags={'amenity': 'bank'})
joyerias    = ox.features_from_place(lugar, tags={'shop': 'jewelry'})
gasolineras = ox.features_from_place(lugar, tags={'amenity': 'fuel'})
estac       = ox.features_from_place(lugar, tags={'amenity': 'parking'})
casas_empen = ox.features_from_place(lugar, tags={'shop': 'pawnbroker'})        # Casas de empeño
licorerías  = ox.features_from_place(lugar, tags={'shop': ['alcohol', 'wine']}) # Licoreras
tiendas_conv= ox.features_from_place(lugar, tags={'shop': 'convenience'})       # OXXO / tiendas
farmacias   = ox.features_from_place(lugar, tags={'amenity': 'pharmacy'})       # Farmacias (robo)
hoteles     = ox.features_from_place(lugar, tags={'tourism': ['hotel', 'motel']})

# ── Factores DISUASORES ───────────────────────────────────────────
print("Descargando POIs disuasores...")
policia     = ox.features_from_place(lugar, tags={'amenity': 'police'})
hospitales  = ox.features_from_place(lugar, tags={'amenity': ['hospital', 'clinic']})
semaforos   = ox.features_from_place(lugar, tags={'highway': 'traffic_signals'})
bomberos    = ox.features_from_place(lugar, tags={'amenity': 'fire_station'})
camaras     = ox.features_from_place(lugar, tags={'man_made': 'surveillance'})   # Cámaras mapeadas
...
heatmap_masked = np.ma.masked_where(heatmap_suave < heatmap_suave.max() * 0.04, heatmap_suave)
...
n_bares   = plotear_poi(bares,       ax, '#FF6B6B', 'o', 11, 6)
n_atms    = plotear_poi(atms,        ax, '#FFD700', 'D',  9, 6)
n_bancos  = plotear_poi(bancos,      ax, '#FFA500', 's', 11, 6)
n_gas     = plotear_poi(gasolineras, ax, '#FF4500', '^', 11, 6)
n_joy     = plotear_poi(joyerias,    ax, '#FF69B4', '*', 14, 7)
n_estac   = plotear_poi(estac,       ax, '#6A6ABA', 's',  7, 4, alpha=0.45)
n_licore  = plotear_poi(licorerías,  ax, '#DA70D6', 'o', 10, 6)
n_conv    = plotear_poi(tiendas_conv,ax, '#87CEEB', 'o',  7, 5, alpha=0.55)
n_farm    = plotear_poi(farmacias,   ax, '#90EE90', '+', 10, 5, alpha=0.6, edge='none', lw=0)
n_hotel   = plotear_poi(hoteles,     ax, '#CD853F', 'h', 12, 6)
n_empen   = plotear_poi(casas_empen, ax, '#FF1493', 'v', 12, 7)

# ── POIs: Factores DISUASORES (encima de todo) ────────────────────
n_pol     = plotear_poi(policia,     ax, '#00FF7F', 's', 16, 9)
n_hosp    = plotear_poi(hospitales,  ax, '#00CED1', 'P', 14, 8)
n_bomb    = plotear_poi(bomberos,    ax, '#FF6347', 'D', 14, 8)
n_sem     = plotear_poi(semaforos,   ax, '#ADFF2F', 'o',  4, 3, alpha=0.35)
...
leg_dis = [
    ...
    entry('o', '#ADFF2F', f'Semáforos ({n_sem})'),
]
